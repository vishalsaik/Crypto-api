...
public class cbdcConsumerController {
	...
	public String createToken(@RequestBody Token tk){
		RestTemplate rTemp = new RestTemplate();
		Token tt = rTemp.postForObject("http://localhost:10000/api/v1/cbdc",tk,Token.class);
		return "Ok created";
	}
	@PutMapping("{i}")
	...
	public Token getToken(@PathVariable("i") long id){
		RestTemplate rTemp = new RestTemplate();
		Token tt = rTemp.getForObject("http://localhost:10000/api/v1/cbdc/"+id,Token.class);
		return tt;
	}
